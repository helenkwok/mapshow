...
fn pass_three_spool_tiles(
    ...
    ElementReader::from_path(input)?.for_each(|element| {
        ...
        let way_restrictions = restrictions_by_from
            .get(&way.id())
            .map(Vec::as_slice)
            .unwrap_or(&[]);
        for (ordinal, pair) in breaks.windows(2).enumerate() {
            ...
            attributes
                .properties
                .insert("segment_id".into(), json!(segment_id));
...
pub fn write_pmtiles_streaming(input: &Path, output: &Path) -> Result<StreamingBuildSummary> {
    let mut prepared = prepare_scratch(input)?;
    let (min_lng, min_lat, max_lng, max_lat) =
        prepared.bounds.unwrap_or((-180.0, -85.0, 180.0, 85.0));
    let metadata = json!({
        "name": "Mapshow game roads",
        "description": "Schema-v3 simulation road tiles generated by the Rust mapshow-roadgen pipeline",
        "attribution": "© OpenStreetMap contributors",
